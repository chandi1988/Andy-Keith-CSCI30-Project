...
def FamTreeLoad(familyName):
	...
	if (len(returnPerson()) != getNumPerson()) and error == False:
		returnPerson().clear()
		getLinkIn().clear()
		print("Error: declared count does NOT match number of person entries")
	elif len(getLinkIn()) > (len(returnPerson()) - 1) and error == False:
		returnPerson().clear()
		getLinkIn().clear()
	...
	while habalnk > 0:
		for i in returnPerson():
			...
			while bilang < len(getLinkIn()):
				...
				elif (findPerson(links[bilang].split()[1]) == None or findPerson(links[bilang].split()[2]) == None) and error == False:
